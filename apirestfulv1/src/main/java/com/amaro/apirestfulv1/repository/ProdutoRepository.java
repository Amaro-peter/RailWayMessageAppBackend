...
public interface ProdutoRepository extends JpaRepository<Produto, Long> {
    ...
    Optional<Produto> recuperarProdutoComIdLock(@Param("id") Long id);

    @Query("select p from Produto p left outer join fetch p.categoria order by p.id")
    List<Produto> recuperarProdutosPorCategoria();
    ...
    @Query(value = "select p from Produto p left outer join fetch p.categoria where p.nome like :nome order by p.id",
    ...
    @Query("select p from Produto p left outer join fetch p.categoria c where c.slug = :slugCategoria order by p.id")
